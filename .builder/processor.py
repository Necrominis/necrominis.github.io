...
def _process_tooltips(paragraph: str) -> str:
	...
	for match in tooltip_pattern.finditer(paragraph):
		...
		processed_paragraph = processed_paragraph.replace(f'[{text}]T({tip})', f'<mark class="tooltip" onclick="toggleTooltip(this);" onmouseleave="closeTooltip(this);">{text}<span class="tip">{tip}</span></mark>', 1)
